...
svm_probs <- attr(predict(svm_model, newdata = Test_processed, probability = TRUE), "probabilities")[,1]  # Probability of positive class
```
...
svm_class <- as.numeric(svm_probs > 0.5)
...
Like our random forest model, the SVM model also has a very high accuracy score of 99.9% using a basic threshold of 0.5
...
# Convert probabilities to class predictions based on a threshold (e.g., 0.5)
stacking_class <- as.numeric(stacking_probs > 0.5)
...
Unsurprisingly, since the ensemble leveraged 2 very strong models (the random forest and the SVM models), it was abale to make strong predictions. The overall accuracy was slightly higher than any of models despite the inaccurate logistic regression. Moving forward we could see that the LR predictor is insignificant and would onyl to serve to add noise to predictions on unseen data thus it would not be included in the ensemble in the application setting.
...
ggplot() +
  ...
  labs(title = "Logistic Regression: Cost of FP = 1 and FN = 10",
       ...
       y = "Cost")
```
Above is the cost analysis of the logistic regression model. The estimated optimal threshold is indicated by the orange point, while the theoretical optimal threshold is indicated by the purple line. Since the logistic regression model is biased towards predicting mushrooms are nontoxic as shown when we were building the model, the optimal threshold is at zero because it minimizes the number of false negatives that occur. In this case, the number of false negatives is minimized to zero because all of the mushrooms in the test set are classified as toxic.

This model is turning out to be a pretty poor model in practice. Because of the model's poor predictive power, our cost analysis is essentially telling us that the optimal decision for foragers is to never eat a foraged mushroom because it cannot tell if the mushroom is poisonous or not. If there are alternative models that produce a better performance than this model, they should indisputably be pursued over this model.
...
computational power was a limitation for the random forest because it takes a long time to build the model