...
Some of the columns (ex. stem-root) are missing a large percentage of values, so we will remove any columns that are missing more than 50% of the values for that variable
...
Explain why we choose this method (check tree notes), why we use the default parameters

why we use RMSE to evaluate model (The square root rather than the square of the error is calculated, meaning that the units are the same as the target variable.)
...
K = 10
folds = sample(rep(1:K, length= nrow(encoded_df) ))

test.RMSE = rep(NA, K)

for (k in 1:K) {
  # Indices of kth fold (the test set for this iteration)
  idx = which(folds == k)
  
  # Training Data
  X.train= subset(encoded_df[-idx, ], select = -c(class_p))
  Y.train = subset(encoded_df[-idx, ], select = c(class_p))$class_p
  
  # Testing Data
  X.test= subset(encoded_df[idx, ], select = -c(class_p))
  Y.test = subset(encoded_df[idx, ], select = c(class_p))$class_p
  
  # Default mtry
  m = sqrt( ncol(X.train) )
  
  # Build Model
  model = ranger(x = X.train, 
                 y = Y.train, 
                 min.bucket = 1, # Default min.obs for ranger()
                 mtry = m) # Default mtry for ranger()
  
  # Predict
  predictions = predict(model, X.test)$predictions
  
  # Calculate kth RMSE
  rmse = sqrt( sum( (Y.test - predictions)^2 ) / length(Y.test) )
...
interpret RMSE (its a vlaue between 0 and 1)
